...
parser$add_argument("--odir", default= "/media/Data/WGBS/LOAD_MCI/Results/CONTROL_MCI_LOAD/CONTROL_MCI_LOAD/test-diagnostic-group-coded/")
...
parser <- ArgumentParser()
parser$add_argument("--idir", default= "/media/Data/WGBS/LOAD_MCI/Results/CONTROL_MCI_LOAD/CONTROL_MCI_LOAD/test-diagnostic-group-coded/", help='Where are the models stored')
parser$add_argument("--odir_pis", default= "/media/Data/WGBS/LOAD_MCI/Results/CONTROL_MCI_LOAD/CONTROL_MCI_LOAD/Outputs/Pis/", help='Where are the models stored')
parser$add_argument("--odir_summaries", default= "/media/Data/WGBS/LOAD_MCI/Results/CONTROL_MCI_LOAD/CONTROL_MCI_LOAD/Outputs/Summaries/", help='Where are the models stored')
...
compute_pi_diffs <- function(X, design.df, pi.df){
    ...
    load.samples <- rownames(X)[design.df$diagnostic_group_coded == 2]
    mci.samples <- rownames(X)[design.df$diagnostic_group_coded == 1]
